 ...
 */
@Service
public class FileNoteManageService implements FileStoreServiceIf {

  private static Logger logger = LoggerFactory.getLogger(FileNoteManageService.class);

  private MongoDaoIf mongoDao;

  private Cache<String, Serializable> newroadCache;
  ...
  public void saveFileResource(final String clientResourceID, CommonFileData fileResource) {
    NoteFileData returnData = getFileResourceById(clientResourceID, null, 1);
    if (returnData == null) {
  ...
  public void updateFileResource(final String clientResourceID, CommonFileData commonFileData) {
    NoteFileData fileResource=(NoteFileData)commonFileData;
    String fileCachePath = fileResource.getFileData().getCacheFileData().getFileCachePath();
    String contentType = fileResource.getFileData().getContentType();
    ...
    if (fileResource.getResourceType() != null) {
      updateMap.put("resourceType", fileResource.getResourceType());
    }
  ...
  public void setFileCache(String clientResourceID, CloudFileData fileCache) {
    newroadCache.set(clientResourceID, fileCache);
  ...
  private NoteFileData getFileResourceById(String clientResourceID, String userID, Integer... status) {
    ...
    NoteFileData FileResource = (NoteFileData) mongoDao.selectOne(FileDataConstant.SELECT_FILE_BY_CLIENTID, map);
    return FileResource;
  }
  ...
  public void setNewroadCache(Cache<String, Serializable> newroadCache) {
